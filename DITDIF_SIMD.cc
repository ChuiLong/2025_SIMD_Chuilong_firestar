...
// 优化的NTT实现
// DIF-NTT实现（正向NTT）- 无需位反转置换
void ntt_dif_simd(int *a, int n, int p) {
        ...
        for (int i = 0; i < n; i += len) {
            for (int j = 0; j < mid; j += SIMD_WIDTH) {
                if (j + SIMD_WIDTH <= mid) {
                    ...
                    sum = mont.normalize4(sum);
                    
                    // 计算a-b
                    uint32x4_t diff = vsubq_u32(vaddq_u32(a_vec, vdupq_n_u32(p)), b_vec);
                    diff = mont.normalize4(diff);
                    
                    // 乘以旋转因子
                    uint32x4_t w_vec = vld1q_u32((uint32_t*)(wn_powers + j));
                    diff = mont.mul4(diff, w_vec);
                    diff = mont.normalize4(diff);
                    ...
                    // 处理剩余元素
                    for (int k = j; k < mid; k++) {
                        uint32x4_t a_vec = vdupq_n_u32(a[i + k]);
                        uint32x4_t b_vec = vdupq_n_u32(a[i + k + mid]);
                        ...
                        // 乘以旋转因子
                        uint32x4_t w_vec = vdupq_n_u32(wn_powers[k]);
                        diff = mont.mul4(diff, w_vec);
                        diff = mont.normalize4(diff);
                        
                        // 保存结果
                        a[i + k] = vgetq_lane_u32(sum, 0);
                        a[i + k + mid] = vgetq_lane_u32(diff, 0);
                    }
                }
            }
        }
...
// DIT-NTT实现（逆NTT）- 无需位反转置换
void ntt_dit_simd(int *a, int n, int p) {
    const int SIMD_WIDTH = 4;
    MontgomeryNEON mont(p);
        ...
        for (int i = 0; i < n; i += len) {
            for (int j = 0; j < mid; j += SIMD_WIDTH) {
                if (j + SIMD_WIDTH <= mid) {
                    ...
                    // 计算b*w
                    uint32x4_t bw = mont.mul4(b_vec, w_vec);
                    bw = mont.normalize4(bw);
                    
                    // 计算a+bw
                    uint32x4_t sum = vaddq_u32(a_vec, bw);
                    sum = mont.normalize4(sum);
                    
                    // 计算a-bw
                    uint32x4_t diff = vsubq_u32(vaddq_u32(a_vec, vdupq_n_u32(p)), bw);
                    diff = mont.normalize4(diff);
...
void poly_multiply_optimized(int *a, int *b, int *ab, int n, int p) {
    ...
    for (int block = 0; block < m; block += BLOCK_SIZE) {
        ...
        // 预取下一个数据块
        if (block + BLOCK_SIZE < m) {
            __builtin_prefetch(&a[block + BLOCK_SIZE], 0);
            __builtin_prefetch(&b[block + BLOCK_SIZE], 0);
        }
